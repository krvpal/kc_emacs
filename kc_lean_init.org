...
(use-package emacs
  ...
  ;; for mac system only - appearance of the title bar
  (when (memq window-system '(mac ns))
    (add-to-list 'default-frame-alist '(ns-transparent-titlebar . t))
    ;; below not needed if using transparent title bar, can also use light
...
(use-package window
  ...
  (setq split-width-threshold 90)        ; this interferes with image-dired when viewing files
...
- Emacs native and in-built way to notifications within Emacs by a pop up buffer/window. See my [[id:4BB12963-EA74-466F-8386-D4FE2ADC23EF][docs]].
...
(use-package appt
  ...
  (setq appt-display-duration 1200)
...
(use-package calfw
  ...
  (:states '(normal visual)
           ...
           "gd" #'cfw:change-view-day
           "gm" #'cfw:change-view-month
           "gt" #'cfw:change-view-two-weeks
           "gw" #'cfw:change-view-week
           ) 
  (:states '(normal visual)
...
(use-package dired
  ...
  (add-hook 'evil-collection-setup-hook (lambda (_mode keymaps)
                                          (general-define-key
...
My docs: [[id:7FE0C051-892A-46AA-A973-1AFFA8EF7D2E][EMMS music in Emacs]]
...
(use-package emms
  ...
  :config
  <<emms-evil-keybindings>>
  <<emms-config>>

  ;; all custom transient functions and keybindings
  <<emms-transients>>
...
*** Keybindings
:PROPERTIES:
:header-args: :tangle no :noweb-ref emms-keybindings
:END:
#+begin_src emacs-lisp

(:states '(normal visual)
         "SPC M" #'emms-smart-browse
         "SPC P" #'kc-emms-smart-go-to-playlist
         "SPC pa" #'emms-add-playlist-directory-tree
         "SPC po" #'emms-play-playlist
         "SPC pp" #'emms-playlist-mode-go-popup
         "SPC pr" #'emms-toggle-repeat-playlist
         "SPC ps" #'emms-shuffle
         "SPC pt" #'emms-toggle-repeat-track
         )
(:states '(normal visual motion)
         :keymaps 'emms-browser-mode-map
         "q" #'emms-browser-bury-buffer
         "gh" #'emms-browser-move-up-level
         "b" #'kc-emms-transient-browse-by
         )
(:states '(normal visual)
         :keymaps 'emms-playlist-mode-map
         "q" #'kc-emms-smart-bury-buffer
         "M-k" #'emms-playlist-mode-shift-track-up
         "M-j" #'emms-playlist-mode-shift-track-down
         )
(:states '(normal visual)
...
*** Evil define keybindings
:PROPERTIES:
:header-args: :tangle no :noweb-ref emms-evil-keybindings
:END:

Evil collection overrides my keys, so the below is additionally needed for keys already occupied by evil-collection packages (see: [[https://github.com/emacs-evil/evil-collection/issues/214][github issue]]). I could not use ~evil-define-key~, because it seems to bind the keys globally and not just for the keymap I pass it, I need to use ~general-define-key~ instead.
...
(add-hook 'evil-collection-setup-hook (lambda (_mode keymaps)
                                        (general-define-key
                                         :states '(normal visual motion)
                                         :keymaps 'emms-browser-mode-map
                                         "s" #'kc-emms-transient-search-by)))

(add-hook 'evil-collection-setup-hook (lambda (_mode keymaps)
                                        (general-define-key
                                         :states '(normal visual motion)
                                         :keymaps 'emms-playlist-mode-map
                                         "s" #'kc-emms-transient-playlist-sort)))
#+end_src
*** Custom functions
:PROPERTIES:
:header-args: :tangle no :noweb-ref emms-kc-functions
:END:
**** Browser custom functions

To be able to browse by 'note', I needed to modify existing functions and do ~advice-add~ to override them.

#+begin_src emacs-lisp

;; to browse-by-note, I needed to modify
;; emms-browser-next-mapping-type and emms-browser-sort-alist
;; and emms-broser-get-face to enable note faces
(defun kc-emms-browser-sort-alist (alist type)
  ...
  (let ((sort-func
         (cond
          ((or
            ...
            (eq type 'info-note)      ; add note here
            (eq type 'info-year)
            (eq type 'info-genre))
...
(defun kc-emms-browser-next-mapping-type (current-mapping)
...
Eg. if CURRENT-MAPPING is currently \\='info-artist, return
 \\='info-album."
  (cond
   ...
   ((eq current-mapping 'info-note) 'info-genre) ; added new for note
   ((eq current-mapping 'info-year) 'info-artist)))

(defun kc-emms-browser-get-face (bdata)
  ...
  (let* ((type (emms-browser-bdata-type bdata))
         (name (cond
                ...
                ((eq type 'info-note) "note") ;; added new for note
...
#+end_src
**** Smart bury and go to playlist
#+begin_src emacs-lisp

(defun kc-emms-smart-bury-buffer ()
  "Bury playlist buffer with browser buffer if displayed"
  (interactive)
  (quit-window nil (get-buffer-window emms-browser-buffer))
  (delete-window)
  )

(defun kc-emms-smart-go-to-playlist ()
  "Go to current playlist with Browse window next to it"
  (interactive)
  (emms-smart-browse)
  (condition-case nil
      (select-window (get-buffer-window emms-playlist-buffer))
    (error (message "No playlist visible"))
    )
  )

#+end_src
**** Function to shorten track description in playlist buffer
#+begin_src emacs-lisp

(defun kc-emms-info-track-description (track)
  "KC function to Return a shorter description of TRACK."
...
*** Config
:PROPERTIES:
:header-args: :tangle no :noweb-ref emms-config
:END:
#+begin_src emacs-lisp
...
(setq emms-mode-line-length-limit 10)
(setq emms-mode-line-mode-line-function 'emms-mode-line-playlist-current)
(setq emms-playlist-mode-window-width 50)

(setq emms-source-file-default-directory (expand-file-name "~/Music/beets"))

(setq emms-tag-editor-tagfile-functions
      '(("mp3" "id3v2" ;; this is for macos
         ((info-artist . "-a")
          (info-title . "-t")
          (info-album . "-A")
          (info-tracknumber . "-T")
...
;; to show only shorter track info in playlist, artist, title [album]
(setq emms-track-description-function 'kc-emms-info-track-description)

(emms-browser-add-category "note" 'info-note)
(emms-browser-make-face "note" "#aaaaff" "#444477" 1.5)

(advice-add 'emms-browser-sort-alist :override #'kc-emms-browser-sort-alist)
(advice-add 'emms-browser-next-mapping-type :override #'kc-emms-browser-next-mapping-type)
(advice-add 'emms-browser-get-face :override #'kc-emms-browser-get-face)

#+end_src
*** Transients 
:PROPERTIES:
:header-args: :tangle no :noweb-ref emms-transients
:END:

#+begin_src emacs-lisp

(transient-define-prefix kc-emms-transient-playlist-sort ()
  "KC Emms sort playlist transient insert with several options"
  [:class transient-columns
          ["Sort by"
           ("n" "Natural order" emms-playlist-sort-by-natural-order)
          ...
          [""
           ...
           ("A" "Album" emms-playlist-sort-by-info-album)
           ("L" "List" emms-playlist-sort-by-list)
           ("C" "Notes or comments" emms-playlist-sort-by-info-note)
           ("T" "File time" emms-playlist-sort-by-file-mtime)
           ]]
  [:class transient-columns
          ["Shuffle"
           ("s" "Shuffle" emms-playlist-shuffle)
           ]
...
(transient-define-prefix kc-emms-transient-browse-by ()
  "KC Emms sort playlist transient insert with several options"
  [:class transient-columns
          ["Browse by"
           ("a" "Artist" emms-browse-by-artist)
           ("g" "Genre" emms-browse-by-genre)
           ("n" "Notes" emms-browse-by-note)
           ("y" "Year" emms-browse-by-year)
           ]
          [""
           ("A" "Album" emms-browse-by-album)
           ]]
  [:class transient-columns
          ["Quit"
           ("q" "Quit" transient-quit-one)]
...
(transient-define-prefix kc-emms-transient-search-by ()
  "KC Emms sort playlist transient insert with several options"
  [:class transient-columns
          ["Search by"
           ("a" "Artist" emms-browser-search-by-artist)
           ("n" "Names" emms-browser-search-by-names)
           ("t" "Title" emms-browser-search-by-title)
           ("p" "Performer" emms-browser-search-by-performer)
           ("c" "Composer" emms-browser-search-by-composer)
           ]
          [""
           ("A" "Album" emms-browser-search-by-album)
           ]]
  [:class transient-columns
          ["Quit"
           ("q" "Quit" transient-quit-one)]
...
(use-package ess
  ...
  ;; the below auto-complete option might be for auto-complete package, might solve R freezes
...
(if (eq system-type 'darwin)
    (use-package exec-path-from-shell
      ...
      ;; any other shell var that needs to be copied over, aprt from
      ;; PATH which is automatic
      (dolist (var '("LANG" "HOMEBREW_CASK_OPTS"))
...
If you use your custom function to call ~image-dired~ recursively, and then want to selectively delete some subdirs and refresh, just call ~image-dired~ again and choose the same folder again, it will refresh with the new list. Make sure the name of the dired buffer is the same folder that you choose.
...
(use-package image-dired
  ...
  (defun kc-image-dired-all-subdirs ()
    "If calling from a dired buffer, make sure you select a subfolder
or go to parent."
    (interactive)
    (let ((dired-listing-switches "-alR")
          (dir (read-directory-name "Select directory")))
      (image-dired dir))
    )
...
(use-package image-mode
  ...
  (:states '(normal visual)
           ...
           "zi" #'image-increase-size
           "zo" #'image-decrease-size
           )
  )
#+end_src
...
(use-package magit
  ...
  (evil-define-minor-mode-key 'normal 'magit-blob-mode "zj" #'magit-blob-next)
  (evil-define-minor-mode-key 'normal 'magit-blob-mode "zk" #'magit-blob-previous)
...
(setq org-preview-latex-image-directory "~/.emacs.d/.kc_cache/latex_previews/")
...
(setq org-emphasis-alist
      '(("*" (bold :foreground "darkred" :background "orange"))
...
(kc-add-to-list-multiple 'org-export-backends '(taskjuggler md pandoc hugo))

;; ox-hugo actually checks if org-preview-latex-image-directory is set
;; or not. Below had to be a different folder than the same cache dir
;; for latex
(setq org-hugo-base-dir (expand-file-name "~/.emacs.d/.kc_cache/hugo_base/"))
...
;; for plantuml babel support, download plantuml using homebrew on mac
;; executable, it also gives a jar if needed
(setq org-plantuml-exec-mode 'plantuml)

;; don't export org with table of content
(setq org-export-with-toc nil)

;; while exporting org to markdown, default starting heading level,
;; useful for when I'm using for Hugo as it won't include iun toc if
;; it doesn't start with level 2
...
(setq org-pandoc-menu-entry 
      '((52 "to html5 and open." org-pandoc-export-to-html5-and-open)
        ...
        (109 "to markdown_mmd and open." org-pandoc-export-to-markdown_mmd-and-open)
        (77 "as markdown_mmd." org-pandoc-export-as-markdown_mmd)
...
;; below to shift headings when exporting using pandoc to markdown
(setq org-pandoc-options-for-markdown_mmd '((shift-heading-level-by . 1)))
...
(:states '(normal visual)
	     ...
	     "SPC mf" #'kc-org-toggle-visible-level-subtrees
...
#+end_src
**** Org expand show or hide sparse tree
#+begin_src emacs-lisp
(defun kc-org-toggle-visible-level-subtrees (prefix level)
  "Used best together with org-sparse-tree. Toggle visibility of direct children for all visible headings at LEVEL.
With a universal argument (C-u), this command hides the subtrees (folds them).
Without an argument, it expands the subtrees to show direct children."
  (interactive "P\nnHeading Level: ")
  (when (and (derived-mode-p 'org-mode) (> level 0))
    (save-excursion
      (goto-char (point-min))
      ;; The core loop remains the same, ensuring it is robust.
      (while (and (not (eobp)) (org-next-visible-heading 1))
        (when (= (org-current-level) level)
          ;; Conditionally call show or hide based on the prefix argument.
          (if prefix
              (org-fold-hide-subtree)
            (org-fold-show-subtree))
          ;; Jump past the current subtree to prevent infinite loops.
          (org-end-of-subtree))))
    ;; Provide simple, clear feedback in the echo area.
    (message (if prefix
                 "Folded matching subtrees."
               "Expanded matching subtrees."))
    )
  )
#+end_src
** Org agenda
...
Inspired by loop habits - the android open source app I've used

- [[https://github.com/ml729/org-habit-stats?tab=readme-ov-file][Github page]]
...
#+end_src
** Plantuml
- Install plantuml using homebrew on mac and then use the executable version
- Be careful to set the ~plantuml-default-exec-mode~, else it will contact server of plantuml.com
- ~Plantuml-mode~ is a major mode, using org syntax, to preview - just call ~plantuml-preview~

#+begin_src emacs-lisp
(use-package plantuml-mode
  :ensure t
  :commands (plantuml-mode plantuml-preview)
  :general
  (:states '(normal)
           :keymaps 'org-mode-map
           "SPC mP" #'plantuml-preview
           )
  ...
  ;; disable contacting server for rendering
...
(use-package poly-R
  ...
  (evil-define-minor-mode-key 'normal 'poly-markdown+r-mode (kbd "SPC me") #'polymode-export)
...
(use-package project
  ...
  ;; below will ensure that any folder with a blank file .project will
  ;; be considered a project root
...
(use-package python
  ...
  (:states '(normal visual)
           ...
           "SPC mi" #'kc-python-df-info-at-point
           ...
           "SPC mr" #'python-shell-send-region
           "SPC mb" #'python-shell-send-buffer
           "SPC mF" #'python-shell-send-defun
           "SPC ms" #'python-shell-send-string
           "SPC Lf" #'eglot-format
           "SPC LF" #'eglot-format-buffer
           "SPC mP" #'run-python
...
With a Universal Arg, it will simply kill all instances"
  ...
  (defun kc-python-df-info-at-point ()
    "Wrap and send df at point with method info() to the Python shell.
This is like R glimpse."
    (interactive)
    (let* ((obj (python-eldoc--get-symbol-at-point))
           (str (concat obj".info()")))
      (python-shell-send-string str)
    ...
    (when (and buffer-file-name (memq major-mode '(python-mode python-ts-mode)))
      (let* (
        ...
        (if (eq exit-status 0)          ; exit status 0 is success, others error
          ...
          (message "Error in Black formatting")
          ))))
...
(use-package spell-fu
  ...
  ;; using :delight won't work, as there is no default lighter set by
  ;; the mode author
  (add-to-list 'minor-mode-alist '(spell-fu-mode " Spell"))
...
** Minor mode for visible search only
#+begin_src emacs-lisp
;; This variable will remember your original setting (e.g., 'open).
(defvar kc-original-search-invisible-value nil
  "Stores the original value of `search-invisible` before it's temporarily changed.")

(define-minor-mode kc-visible-search-mode
  "A minor mode to temporarily set `search-invisible' to nil.
When turned on, searching is restricted to visible text only.
When turned off, the original value of `search-invisible' is restored."
  :init-value nil
  :lighter " VisSrch"
  ;; This block of code runs every time the mode is toggled.
  (if kc-visible-search-mode
      ;; --- Code to run when the mode is turned ON ---
      ;; When the mode is active, we want to search VISIBLE text only.
      ;; To do this, we must set `search-invisible` to `nil`.
      (progn
        ;; 1. Remember the current value (e.g., 'open).
        (setq kc-original-search-invisible-value search-invisible)
        ;; 2. Set the value for visible-only searching.
        (setq search-invisible nil))
    ;; --- Code to run when the mode is turned OFF ---
    ;; When the mode is inactive, we want to restore the original behavior.
    (progn
      ;; 1. Restore the exact value we saved.
      (setq search-invisible kc-original-search-invisible-value))))

;; (Optional but recommended) Create a user-friendly alias for the command.
(defalias 'kc-toggle-visible-search #'kc-visible-search-mode)
#+end_src
** Startup splash or dashboard screen

Borrowed from doom Emacs doom-dashboard

- To suppress ~q~ keybinding from running ~quit-window~
  
  This mode is derived from ~special-mode~, and for whatever reason it meant that the keybinding ~q~ in normal mode was leading to ~quit-window~. I could not disable this as it was part of some minor mode keymap, one of the ideas was to create my own minor-mode but then this minor mode should be globally enabled so it is first in the order in the alist =minor-mode-map-alist=. It is cumbersome to change a minor-mode's keybinding depending on what major mode is active.

  I had to finally use ~evil-local-set-key~ and set ~q~ to function ~ignore~, which creates a buffer-local keymap and thus overrides any minor-mode keybinding.
