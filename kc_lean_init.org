...
(use-package ess
  ...
  (:states '(normal visual)
           ...
           "SPC mF" #'kc-r-styler-format-file
  ...
  (defun kc-r-styler-format-buffer ()
    ...
    (when (memq major-mode '(ess-r-mode))
      (let* ((cmd "Rscript -e \"styler::style_text(readLines('stdin'))\"")
             (original-point (point))  ; Save current cursor position
             (kc-r-exit-status))
        ;; Create a temporary buffer to hold the formatted content
        (with-temp-buffer
          (insert-buffer-substring-no-properties (current-buffer))
          (setq kc-r-exit-status (shell-command-on-region (point-min) (point-max) cmd (current-buffer) t))
